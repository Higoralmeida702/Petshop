...
namespace Petshop.Domain.Model
{
    public class Animal
    {
        ...
        public Animal(string nome, string cor, string raca, decimal peso, decimal comprimento, int clienteId, AnimalCategoriaEnum animalCategoria, GeneroEnum genero)
        {
            ...
            DomainExceptionValidations.When(clienteId <= 0, "O id do cliente deve ser valido");

            ValidateDomain(nome, cor, raca, peso, comprimento, clienteId, animalCategoria, genero);
        }

        public void ValidateDomain(string nome, string cor, string raca, decimal peso, decimal comprimento, int clienteId, AnimalCategoriaEnum animalCategoria, GeneroEnum genero)
        {
            Nome = nome;
            Cor = cor;
            Raca = raca;
        ...
        public void Update(string nome, string cor, string raca, decimal peso, decimal comprimento, int clienteId, AnimalCategoriaEnum animalCategoria, GeneroEnum genero)
        {
            ValidateDomain(nome, cor, raca, peso, comprimento, clienteId, animalCategoria, genero);
        }
    }
